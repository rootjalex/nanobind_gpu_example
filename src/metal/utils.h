...
static NS::SharedPtr<MTL::CommandQueue> getCommandQueue() {
    static NS::SharedPtr<MTL::CommandQueue> queue =
        NS::TransferPtr<MTL::CommandQueue>(getDevice()->newCommandQueue());
    return queue;
}
...
void setBufferOrBytesArgument(MTL::ComputeCommandEncoder* encoder,
                              const Arg& arg, uint32_t index) {
    using BareT = std::remove_cv_t<std::remove_pointer_t<Arg>>;

    if constexpr (std::is_pointer_v<Arg> &&
                  std::is_base_of_v<MTL::Buffer, BareT>) {
        ...
        // TODO: for passing structs, may also want:
        // std::is_trivially_copyable_v<BareT> && small size (< 4KB?) Scalar
        // type
        encoder->setBytes(&arg, sizeof(Arg), index);
    } else {
        static_assert(sizeof(Arg) == 0,
                      "Only MTL::Buffer* or scalar types are supported");
    }
}

// --- Variadic helper ---
template <typename... Args>
void setBufferOrBytes(MTL::ComputeCommandEncoder* encoder,
                      const Args&... args) {
    uint32_t index = 0;
...
void launchFunction(NS::SharedPtr<MTL::Device> device,
                    ...
                    const uint32_t n_threads, const Args&... args) {
    auto commandBuffer = getCommandBuffer();
    // Pretty sure this does not need to me memory-managed, tied to
    // commandBuffer lifetime.
    MTL::ComputeCommandEncoder* encoder =
        commandBuffer->computeCommandEncoder();

    NS::Error* error = nullptr;
    NS::SharedPtr<MTL::ComputePipelineState> pipelineState =
        NS::TransferPtr<MTL::ComputePipelineState>(
...
template <typename T>
MTL::Buffer* makeDeviceBuffer(NS::SharedPtr<MTL::Device> device,
                              const size_t& size) {
    return device->newBuffer(
        NS::UInteger(size * sizeof(T)),
